...
app.get("/api/health", (c) =>
	c.json({
		...
		rules: {
			...
			on_unconfirmed_collect: "cancel_offer_after_24h",
			handover:
				"collector_marks_collected_then_poster_confirms; progressive_daily_limit_1_to_5",
			recurring:
				...
				.then((result) => {
					console.log(
						JSON.stringify({
							...
							unconfirmed_scanned: result.unconfirmed_scanned,
							unconfirmed_released: result.unconfirmed_released,
							unconfirmed_cancelled: result.unconfirmed_cancelled,
							auth: result.auth,
						}),
					);
				})
